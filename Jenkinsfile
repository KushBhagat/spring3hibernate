pipeline {
	...
	parameters {
		...
		string (name: 'GIT_URL',defaultValue: 'https://github.com/KushBhagat/spring3hibernate', description: '', trim: false)
}
	...
	stages 
	{
               stage('Checkout') {
                     steps 
					 {   
                      checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'http://github.com/KushBhagat/spring3hibernate.git']]])
                     }    
                }
